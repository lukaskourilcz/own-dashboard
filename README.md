...
- **Work** — portfolio overview, explainable project health, guided weekly planning (last week's calendar time by client/project/admin channel, finished focus tasks, carry-forward, next week's objectives, summary), project workspaces, client opportunities, organizations/clients, Career, and the canonical invoice workflow.
...
8. `20260917150000_cron_heartbeats.sql` — per-cron push-monitor URL and last-success timestamp, so a scheduled job that stops running is visible instead of silent.
