...
- **Library** — BlockNote notes with full-context copy and automatic stale-empty cleanup; prompts grouped by the kind of job (design, audit, competition, UX and UI, analysis, documentation, new project, SEO, marketing) with the links an agent should open and a copy preview that fills in a project and its links; Tools, which lists by itself what the active projects' repositories use (their `about-project.md`, or `package.json` as a fallback) next to the library links added by hand with what each does, its status, cost and how it helps each project; masonry-grouped enriched links showing which projects use them; and IG TIPS, the tips saved from Instagram Reels and research, grouped by topic, each card saying in plain sentences what the tip is and how to use it. Project Knowledge renders the linked repository's `about-project.md` as formatted Markdown.
- **Settings** — database-synchronized appearance, navigation, task density, CV links, active GitHub projects with an Own/Freelance choice for each active one, integrations, notification controls, own-only exports, legacy archive download, and account controls.
...
Library: Notes · Prompts · Tools · Links · IG TIPS
Settings
