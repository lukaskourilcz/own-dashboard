...
- [ ] **Contextual AI:** set `ANTHROPIC_API_KEY`. Optionally set `ANTHROPIC_BASE_URL` and the three centralized model overrides (`AI_INTENT_MODEL`, `AI_ENRICHMENT_MODEL`, `AI_SYNTHESIS_MODEL`). Without a key, briefs, copilots, enrichment, and knowledge review do not run; link enrichment falls back to the anonymous Jina Reader for title and description only. The natural-language quick-add box was removed in `18cf42f`, so `/api/quick-add` and its intent-routing to `/api/ai/search` are no longer wired to any UI. `[imp:4]` `[owner:me]`
