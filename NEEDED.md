...
- [ ] **Apply the IG TIPS migration, then the tip content** — `20260926140000_ig_tips.sql` adds `ai_links.tip_group` and `ai_links.tip_summary` with their checks and changes no policy. Apply it with the deploy: IG TIPS reads without it, but adding or editing a tip fails until both columns exist. Then run the separate data file prepared with this change, 73 updates keyed by id that give each tip its topic and card text. It stays out of the repository because it is your library content. `docs/migration-guide.md` has the checks. [imp:4] [owner:me] [time:10m] [kind:deploy]
- [ ] **Read the IG TIPS cards once** — each tip's two to four sentences were written from its own stored notes, sources and review, not by watching the Reels again. Correct any card that misreads its original with Edit. One title promises a second step that the stored notes never describe; the session report names it. [imp:2] [owner:me] [time:30m] [kind:content]
- [ ] **Check the repository list in Tools after the deploy** — the agent session could not read three active projects' repositories because they are not public, so their stacks are unverified. Open Tools → Found in repositories → Repositories. A row that says the repository is not visible to the connected account needs a wider GitHub grant; the other rows need nothing. [imp:2] [owner:me] [time:10m] [kind:deploy]
- [ ] **Write DNESKAi's and devShark's stack bullets as `Name — what it does`** — Tools reads the Tech stack and Third-party libraries bullets in that form and skips `**Label:** value` lines. All ten of DNESKAi's bullets are label lines, so Tools falls back to its `package.json` and lists eight packages without a description; devShark has six label lines that Tools leaves out. [imp:2] [owner:me] [time:20m] [kind:content]
...
- [x] **Align the Supabase migration history before the next `db push`** — done. The production migration list read on 2026-09-26 holds exactly the 37 file versions from `20260721165419` to `20260925210000`: the six kickoff migrations under their file versions `20260925090000`–`20260925090500`, and none of the nine `20260607…` rows. `npx supabase db push --linked --dry-run` should now list only `20260926140000_ig_tips.sql`. [imp:4] [owner:me] [time:20m] [kind:setup]
...
- [x] **Apply the five pending migrations** — applied. The same list on 2026-09-26 records `20260925200000_fix_project_parent_policies`, `20260925200100_project_engagement_replaces_scope`, `20260925200200_project_competition_tab`, `20260925200300_drop_link_project_references` and `20260925210000_restrict_purge_old_cron_runs` under their file versions. `docs/migration-guide.md` still has the checks for each. [imp:4] [owner:me] [time:25m] [kind:deploy]
...
- [ ] **Decide whether umyjemefasadu is client work** — the 2026-09-16 import marked it `work`, the kickoff left it `own`, and `projects.engagement` now decides. It is inactive, so it shows nowhere today. If it was a client job, turn it on in Settings → Active projects and choose Freelance on its row; the project form's Engagement field does the same. [imp:1] [owner:me] [time:5m] [kind:decision]
...
- [ ] **Run the post-deploy smoke test** — once `main` has deployed and `20260926140000_ig_tips.sql` is applied, walk `docs/external-setup.md` §9, including the second account that must not read or link the first account's records. [imp:4] [owner:me] [time:45m] [kind:deploy]
...
- [ ] **Repository knowledge:** add `about-project.md` to every active project repository. Include a short opening summary plus `## Tech stack` and `## Third-party libraries` list sections; write each entry as `Name — what it does`. Use Project → Knowledge → Check current info to see it rendered; Tools lists what those two sections name. `[imp:3]` `[owner:me]` `[time:30m]` `[kind:content]`
