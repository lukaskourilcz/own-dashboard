...
- [ ] **Back up Supabase and apply the four pending migrations** with `npx supabase db push --linked`; run `20260721165419_professional_restructure_core.sql`, then `20260721165421_remove_legacy_personal_scope.sql`, `20260722150000_atomic_inbox_routing.sql`, and `20260722190000_operational_workflow_extensions.sql`. Do **not** rerun `supabase/schema.sql` on an existing project. `[imp:5]` `[owner:me]`
...
- [ ] **Brand-media production:** Higgsfield is connected, but the 2026-07-22 audit found zero usable MCP trial credits. Before paying, compare at least three current low-cost or free generators using primary pricing, licensing, privacy, and format documentation. Use a safe documented fallback if one qualifies; otherwise [add a Higgsfield MCP credit allowance](https://higgsfield.ai/mcp-credits?show_modal=auto_refill&source=mcp). Resume from `docs/design/generated-media-manifest.json` and never publish unreviewed output. This is a one-off design-production dependency, not a runtime application dependency. `[imp:2]` `[owner:me]`
