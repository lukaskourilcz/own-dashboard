...
- [ ] **Scheduled jobs:** set a strong `CRON_SECRET` in Vercel and verify the three schedules in `vercel.json` call bank sync at 06:00 UTC, payment matching at 06:30 UTC and renewal warnings at 07:00 UTC. Job boards are refreshed only by Career's **Check for new offers** button, so there is no scraping schedule to verify. Add `HEARTBEAT_URL` if you want an external success monitor. `[imp:4]` `[owner:me]`
