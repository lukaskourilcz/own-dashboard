...
- [ ] **Set the release entry's date to the merge day** — the entry covering issues #64 to #73 is dated 2026-09-17 because all eleven commits on the branch are dated 2026-09-16 and the existing entry already owns that date. `tests/lib/changelog.test.ts` forbids two entries on one date and requires strictly descending dates, so the merge day has to be 2026-09-17 or later; merging on 2026-09-16 means folding the two entries into one instead. Change the date to the day the branch actually merges, regenerate `CHANGELOG.md` and re-run `npx vitest run tests/lib/changelog.test.ts`. `[imp:2]` `[owner:me]` `[time:10m]` `[kind:content]`
...
- [ ] **Decide how the baseline gets fixed** — today's `supabase/schema.sql` fails on a fresh database at line 1969 with `column "invoice_id" does not exist`. The mirrored `transactions_unmatched_income_idx` has a predicate over `transactions.invoice_id`, a column that `20260721165419_professional_restructure_core.sql` creates later; `create index if not exists` guards the index name only, never the columns in its predicate. The two statements after it never run either, so a fresh install also silently loses `subscriptions.amount_confirmed_on`. An older blocker sits above it: commit b66ab79 mirrored the whole `saved_job_positions` block into the baseline with bare `create policy` in both files, so even with the newer block gone the chain aborts on duplicate policies. The recommendation, verified by executing both paths end to end against a real Postgres: delete the two mirrored blocks — lines 1875-1994 and 1471-1504 — leaving an 1,840-line baseline that runs to exit 0, then all 25 migrations under `ON_ERROR_STOP=1`, 25 of 25 at exit 0, with the final state matching the Verify list in `docs/migration-guide.md`. The alternative, consolidating every post-20260721 migration into the baseline, is not tractable: roughly 247 non-idempotent statements collide, and a consolidated baseline cannot be verified without production credentials. `[imp:5]` `[owner:me]` `[time:30m]` `[kind:decision]`
- [ ] **Write the no-mirroring rule into the baseline header, and turn README.md:114 into a procedure** — the same bug has now happened twice by two different mechanisms, so the rule belongs in `supabase/schema.sql` itself: a migration's block is never copied back into the baseline. README.md:114 currently carries one vague clause about initializing the base schema; it needs to be an ordered procedure — run `supabase/schema.sql` once, create the one owner account in Supabase Auth, then apply every migration in timestamp order. Step 2 is not optional and was reproduced: `20260908063200_seed_owner_saved_positions.sql` raises "Expected one OwnDashboard owner; refusing to seed saved positions" when `auth.users` holds anything other than exactly one row. `docs/migration-guide.md:46` is wrong as well — there is no `supabase/config.toml`, so `npx supabase db reset` needs `supabase init` first and never runs the baseline anyway. Do this in the same change as the decision above. `[imp:3]` `[owner:me]` `[time:30m]` `[kind:setup]`
