...
6. Run `/dev-preview` visual/accessibility checks from the visual-QA skill, verify PWA metadata/icons/install affordance, and inspect media sizes/usage/fallbacks. When Higgsfield is unavailable or credit-blocked, verify the media workflow researched at least three current low-cost/free alternatives from primary sources and recorded provider, cost/license, privacy, watermark, format, and provenance evidence before any fallback asset ships.
