...
description: Design, implement, or review OwnDashboard UI, shared components, status presentation, responsive behavior, copy, charts, tables, forms, motion, or accessibility. Use whenever a screen or visual pattern changes in authenticated, login, PWA, invoice-adjacent, empty, or error surfaces.
---
...
4. Use the native system stack (`-apple-system`, BlinkMacSystemFont, then Geist) for UI text and Geist Mono for technical values, tabular figures for money/dates/counts, modest headings, modest radii, border hierarchy, and elevation only for floating surfaces.
...
6. Reuse `PageHeader`, `Card`, `Button`, `EmptyState`, `StatusBadge`, `EntityBadge`, `Metric`, dialogs, selects, and existing domain components. Create an abstraction only after finding at least two real consumers.
