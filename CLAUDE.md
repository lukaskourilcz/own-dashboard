...
Higgsfield is preferred for approved brand media but is currently credit-blocked. MCP presence is not sufficient: check workspace selection, balance, billing/trial status, and a cost-only preflight before production. When Higgsfield is unavailable or has no usable credits, do not wait: search current primary provider documentation for at least three low-cost or free alternatives and compare real cost/free quota, output rights, watermark, privacy/retention, formats, quality fit, and credential requirements. Select and document the safest viable fallback, but do not register, purchase, expose credentials, or upload private data without owner authorization. Preserve authentic `/dev-preview` screenshots, never generate UI, and leave media deferred without placeholders if no alternative qualifies.
