...
Design for “calm operational intelligence” through the approved macOS-native operational-cartography system: a graphite desktop, a contained 1360 px app window, 224 px translucent dark sidebar, 52 px toolbar, compact white modules, system UI typography, tabular figures, and restrained semantic macOS accents. Desktop window chrome is an OwnDashboard presentation device, not an Apple mark. Search before creating; reuse semantic tokens and shared components; never add filler media, fake UI, glow, generic decorative gradients, sparkles/metrics, nested-card walls, or a second icon system. Gradients are limited to the graphite desktop, primary control, and avatar treatment. Preserve restrained motion, reduced-motion support, and EN/CS parity. Validate long content, non-happy paths, mobile widths, keyboard flow, and WCAG 2.2 AA behavior.
