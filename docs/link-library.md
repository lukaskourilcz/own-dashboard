...
The `/links` route uses the existing owner-scoped `ai_links` and `ai_categories` tables. Historical table and component names are retained to avoid a storage migration; the user-facing feature is a general resource library. It lists link records only: idea records are the tips in IG TIPS (`/ig-tips`), and a category holding nothing but tips is left out. Link and category mutations retain the existing TanStack Query invalidation and own-only Supabase policies.
