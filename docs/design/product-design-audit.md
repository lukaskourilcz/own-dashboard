...
| Work review | Explainable health, a compact metric band, and a guided five-step weekly planning flow | Channel accuracy depends on event titles naming a project or client, so the unmatched count has to stay visible | Keep the attention band and compact metrics; show one step at a time with an honest unmatched count and a legible disconnected state |
