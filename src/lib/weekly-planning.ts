 ...
 * A project is client work when it is a client engagement (`scope === "work"`)
 * or belongs to an organization, and own-project work otherwise. Anything that
...
export function classifyEvent(
  ...
  for (const project of refs.projects) {
    const needles = [project.name, project.slug].filter(Boolean);
    if (!needles.some((needle) => containsWord(haystack, normalize(needle)))) continue;
    const isClientWork = project.scope === "work" || Boolean(project.organization_id);
