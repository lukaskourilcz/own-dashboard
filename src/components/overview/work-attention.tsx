...
export function WorkAttention({ opportunities, onOpen }: { opportunities: ClientOpportunity[]; onOpen: () => void }) {
  ...
  const items = opportunities
    ...
    .slice(0, 4);
  return <Card>
    <CardHeader><CardTitle className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-2"><span className="mac-icon-tile"><BriefcaseBusiness /></span>{p.attention}</span><button type="button" onClick={onOpen} className="text-xs font-normal text-foreground-muted hover:text-foreground">{p.opportunitiesTitle}</button></CardTitle></CardHeader>
