...
import { childProjects, groupPortfolio, isPortfolioProject, portfolioEntryFor } from "@/lib/portfolio";
...
/**
 * Competition — every competitor recorded for every project, grouped by the
 * portfolio registry's categories, with search, project and type filters.
 */
export function CompetitionPanel({ projects, competitors, setCompetitors, onOpenProject }: {
  ...
  // Portfolio projects first (with their subsections) in registry order, then
  // client projects, then any other project that already has research
  // attached, so nothing recorded is unreachable.
  const orderedProjects = useMemo(() => {
    const active = projects.filter((project) => project.is_active && (isPortfolioProject(project) || projectEngagement(project) === "client"));
  ...
  const groupsToRender = orderedProjects.filter((project) => byProject.has(project.id) || (!filtering && isPortfolioProject(project)));
