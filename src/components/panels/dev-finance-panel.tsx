 ...
 * hosting spend, its split between own projects and client work,
          ...
          <section className="surface-band p-4 sm:p-5">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="grid gap-2 sm:grid-cols-3">
                <Metric label={f.ownShare} value={formatCurrency(summary.ownMonthly, displayCurrency)} detail={shareOf(summary.ownMonthly)} icon={FolderKanban} />
                <Metric label={f.clientShare} value={formatCurrency(summary.clientMonthly, displayCurrency)} detail={shareOf(summary.clientMonthly)} icon={Handshake} />
              ...
              <table className="w-full min-w-[720px] text-left">
                <thead className="border-b border-border bg-surface-secondary text-[11px] text-foreground-muted">
                  <tr>
                    ...
                    <th scope="col" className="px-3 py-2.5 font-medium">{t.projects.engagement}</th>
                ...
                <tbody className="divide-y divide-border">
                  {summary.byProject.filter((row) => row.recurringMonthly > 0 || row.paidTotal > 0).map((row) => {
                    const engagement = projectEngagement(row.project);
                    ...
                    return (
                      <tr key={row.project.id} className="hover:bg-surface-hover">
                        ...
                        <td className="px-3 py-2.5"><StatusBadge value={engagement} label={engagement === "client" ? t.projects.engagementClient : t.projects.engagementOwn} tone={engagement === "client" ? "information" : "neutral"} /></td>
