...
The Links section provides categorized resources with compact expandable cards, pricing dots and a visible legend, combined search/category/pricing/connection filters, and existing edit/delete actions. Storage, pricing boundaries, URL handling and verification are described in [Resource library](docs/link-library.md).

Every link or idea can be connected to one or more projects with a status of *used* or *planned*. The connection is one own-only row in `ai_link_projects`; the library card shows a connected tag naming the projects, the connection filter narrows the library to connected or unconnected records, and the project workspace gains a References tab that lists the same rows from the project side and can connect more. Prompts gains a *From ideas* subsection that drafts a prompt from each idea's own fields (description, relevance, rationale, sources) without any model call; adding a draft files it under Mine, linked to the first project the idea's relevance resolves to. A project can also carry a Google Drive video link, shown as a Video action in its workspace header; the file itself never enters the database. Details are in [Links and Ideas](docs/links-and-ideas.md).
