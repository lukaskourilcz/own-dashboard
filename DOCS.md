...
The public surface remains a private sign-in entry, not a marketing site. It uses the centralized brand mark, explains the own-only operating model, keeps Google sign-in primary, handles callback errors, and exposes a media seam without shipping placeholder artwork. A live Higgsfield MCP audit reached the workspace, catalog, and cost preflight, but the active trial had zero usable generation credits and returned no assets. A future media pass must research current low-cost or free alternatives from primary sources and may use only a documented option with acceptable rights, privacy, watermark, and format behavior. The exact prompts and attempts remain in `docs/design/generated-media-manifest.json`; production metadata does not reference proposed files.
