...
Applied also has a stage board beside its list: five columns over the existing `job_applications.status` plus the `saved_job_positions` queue, where a drag writes through the same `update_job_application_progress` RPC and a saved position still becomes an application only through the review dialog. Every card carries a stage menu, so the board needs no pointer. A due follow-up now also appears on the Work overview and in the Home hero's follow-up column, derived from the loaded records rather than written to `notifications`. Structured `contact_name` and `contact_email` arrive with the additive `20260917190000_job_application_contacts.sql`; until it is applied the dialog says the fields need it.
